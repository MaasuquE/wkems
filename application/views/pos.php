                    ...
                    <div class="col-sm-12" style="overflow-y:auto;border:1px solid #337ab7;" >
                      <table class="table table-condensed table-bordered table-striped table-responsive items_table" style="">
                        <thead class="bg-primary">
                          ...
                          <?php /*?><th width="10%"><?= $this->lang->line('tax'); ?></th><?php */?>
...
function addrow(id){
    ...
    var str=' <tr id="row_'+rowcount+'" data-row="0" data-item-id='+item_id+'>';/*item id*/
        ...
        /*Tax amt*/
        str+='<td style="display:none;" id="td_'+rowcount+'_11"><input data-toggle="tooltip" title="Click to Change" id="td_data_'+rowcount+'_11" onclick="show_sales_item_modal('+rowcount+')" name="td_data_'+rowcount+'_11" type="text" class="form-control no-padding pointer min_width" readonly value="'+tax_amt+'"></td>';
...
function increment_qty(item_id,rowcount){
  ...
  if(parseFloat(item_qty)<parseFloat(stock)){
    ...
    $("#item_qty_"+item_id).val(item_qty.toFixed(3));
  }
  make_subtotal(item_id,rowcount);
}
//DECREMENT ITEM
function decrement_qty(item_id,rowcount){
  var item_qty=$("#item_qty_"+item_id).val();
  if(item_qty<=1){
    $("#item_qty_"+item_id).val((1).toFixed(3));
    return;
  }
  $("#item_qty_"+item_id).val((parseFloat(item_qty)-1).toFixed(3));
  make_subtotal(item_id,rowcount);
}
...
function make_subtotal(item_id,rowcount){
  ...
  $("#td_data_"+rowcount+"_4").val(parseFloat(subtotal).toFixed(3));
  final_total();
}
...
function set_total(tot_qty=0, tot_amt=0, tot_disc=0, tot_grand=0){
  ...
  $(".tot_amt   ").html((round_off(tot_amt).toFixed(3)));
  $(".tot_disc  ").html((round_off(tot_disc).toFixed(3)));
  $(".tot_grand ").html((round_off(tot_grand)).toFixed(3));
}
...
function adjust_payments(){
  ...
  $(".sales_div_tot_amt  ").html((round_off(total)).toFixed(3));
  $(".sales_div_tot_discount ").html((parseFloat(round_off(discount_amt))).toFixed(3)); 
  $(".sales_div_tot_payble ").html((parseFloat(round_off(total-discount_amt))).toFixed(3)); 
  $(".sales_div_tot_paid ").html((round_off(paid_amount)).toFixed(3));
  $(".sales_div_tot_balance ").html((parseFloat(round_off(balance))).toFixed(3)); 
  
  /**/
  $(".sales_div_change_return ").html((change_return).toFixed(3)); 
