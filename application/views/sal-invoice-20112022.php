...
<body class="hold-transition skin-blue sidebar-mini">
  <div class="wrapper">
      ...
      <section class="invoice">
              ...
              <table class="table table-striped records_table table-bordered">
                <thead class="bg-gray-active">
                  <tr>
                    ...
                    <?php /* ?> <th><?= $this->lang->line('tax'); ?></th>
                    <th><?= $this->lang->line('tax_amount'); ?></th> <?php  */ ?>
                ...
                <tbody>
                  ...
                  foreach ($q2->result() as $res2) {
                    ...
                    if($res2->discount_type == 'Percentage'){
                        $discount .= ' %';
                    }
                    $discount_amt = (empty($res2->discount_amt) || $res2->discount_input == 0) ? '0' : $res2->discount_amt . "";
                    echo "<tr>";
                    echo '<td><input type="checkbox" name="hide_item" value="' . $res2->id . '" class="hide_item" style="position: absolute; top: -10%; left: -10%; display: block; width: 120%; height: 120%; margin: 0px; padding: 0px; background: rgb(255, 255, 255); border: 0px; opacity: 0;"></td>';
                    echo "<td>" . ++$i . "</td>";
                    echo "<td>";
                    echo $res2->item_name;
                    echo (!empty($res2->description)) ? "<br><i>[" . nl2br($res2->description) . "]</i>" : '';
                    echo "</td>";
                    echo "<td class='text-right'>" . $CI->currency(number_format($res2->price_per_unit, 3, '.', '')) . "</td>";
                    echo "<td>" . $res2->sales_qty . "</td>";
                    echo "<td class='text-right'>" . $CI->currency(number_format(($res2->price_per_unit * $res2->sales_qty), 3, '.', '')) . "</td>";
                    ...
                    echo "<td class='text-right'>" . $CI->currency(number_format($res2->unit_total_cost, 3, '.', '')) . "</td>";
                    echo "<td class='text-right'>" . $CI->currency(number_format($res2->total_cost, 3, '.', '')) . "</td>";
                    echo "</tr>";
                ...
                <tfoot class="text-right text-bold bg-gray">
                  <tr>
                    ...
                    <td><?= $CI->currency(number_format($tot_sales_price, 3, '.', '')); ?></td>
                    <td class="text-left"><?= $tot_qty; ?></td>
                    <td>-</td>
                    <?php  /*?> <td>-</td>
                    <td><?= $CI->currency(number_format($tot_tax_amt, 3, '.', '')); ?></td><?php  */ ?>
                    <td>-</td>
                    <td><?= $CI->currency(number_format($tot_discount_amt, 3, '.', '')); ?></td>
                    <td>-</td>
                    <td><?= $CI->currency(number_format($tot_total_cost, 3, '.', '')); ?></td>
                  </tr>
                </tfoot>
              </table>
            </div>
                      ...
                      <tbody>
                        ...
                        if (isset($sales_id)) {
                          ...
                          if ($q3->num_rows() > 0) {
                            ...
                            echo "<tr class='text-right text-bold'><td colspan='4' >Total</td><td>" . number_format($total_paid, 3, '.', '') . "</td></tr>";
                          } else {
        ...
        <div class="row no-print">
          <div class="col-xs-12">
            ...
            <a href="<?php echo $base_url; ?>sales/print_invoice/<?php echo  $sales_id ?>" data-url="<?php echo $base_url; ?>sales/print_invoice/<?php echo  $sales_id ?>" target="_blank" class="btn btn-warning print_btn">
            ...
            <a href="<?php echo $base_url; ?>sales/pdf/<?php echo  $sales_id ?>" target="_blank" class="btn btn-primary pdf_btn" style="display:none">
            ...
            <a onclick="send_mail_modal()" class="btn btn-success">
              <i class="fa fa-envelope"></i>
              Mail
            </a>


            <a href="<?php echo $base_url; ?>sales/quotation/<?php echo  $sales_id ?>" target="_blank" class="btn btn-primary">
              <i class="fa fa-file-o"></i>
              Quotation
            </a>
            <?php
            echo '<a style="cursor:pointer" class="btn btn-warning  " title="Delivery note " onclick="get_delivery_note(\'' . base_url() . 'sales/getDeliveryNote/' .  $sales_id . '\',\'' . base_url() . 'sales/setDeliveryNote/' .  $sales_id . '\',\'' . base_url() . 'sales/deliveryNote/' .  $sales_id. '\')" >
            <i class="fa fa-fw fa-file-text text-white"></i>Delivery note
          </a>';
            ?>
    ...
    <?php include "modals/modal_remarks_mail.php"; ?>
    <!-- /.content-wrapper -->
  ...
  <!-- ./wrapper -->
  <?php include "modals/modal_invoice_mail.php"; ?>
  <!-- SOUND CODE -->
  <?php include "comman/code_js_sound.php"; ?>
  <!-- TABLES CODE -->
  <?php include "comman/code_js_form.php"; ?>
  <script type="text/javascript">
    $(".hide_item").on('ifChanged', function(e) {
      $(this).trigger("change", e);
      var print_url = $('.print_btn').data('url');
      var val = [];
      $('.hide_item:checked').each(function(i) {
        val[i] = $(this).val();
      });
      print_url += '?hide_items=' + val;
    ...
    function send_mail_modal() {
      $('#invoice-mail-modal').modal('toggle');
    }

    function sendInvoiceMail() {
      var invoice_email = $('#invoice-email').val();
      var val = [];
      $('.hide_item:checked').each(function(i) {
        val[i] = $(this).val();
      });

      $.post('<?= base_url(); ?>/sales/sendInvoiceMail/<?= $sales_id; ?>?hide_items=' + val, {
        invoice_email: invoice_email
      }, function(result) {
        $('#invoice-email').val('');
        if (result == 'failed') {
          toastr["error"]("Sorry! Mail not sent.Try again!");
  ...
  <script>
    ...
    function get_delivery_note(url, submitUrl, redirectUrl) {
      var val = [];
      $('.hide_item:checked').each(function(i) {
        val[i] = $(this).val();
      });
      $.post(url, function(result) {
        $('#sales_id_for_remarsk').val(submitUrl);
        $('#ramak_id_redirect_url').val(redirectUrl+'?hide_items='+val);
        $('#remarsk-text').val(result);
        $('#remarsk-hide').val(val);
        $('#sales-remarks-modal').modal('show');
      });
    }

    function print_delivery_note() {
      var submitUrl = $('#sales_id_for_remarsk').val();
      var redirectUrl = $('#ramak_id_redirect_url').val();
      var remarks = $('#remarsk-text').val();
      var data = new FormData($('#sales-remarks-form')[0]);
      $.ajax({
        type: 'POST',
        url: submitUrl,
        data: data,
