        ...
        <label for="from_date" class="col-sm-2 control-label"><?= $this->lang->line('from_date'); ?></label>
          ...
          <label for="to_date" class="col-sm-2 control-label"><?= $this->lang->line('to_date'); ?></label>
            ...
            <span id="purchase_date_msg" style="display:none" class="text-danger"></span>
                ...
                <div class="form-group">
          <label for="item_id" class="col-sm-2 control-label"><?= $this->lang->line('services_technician_name'); ?></label>
          ...
          <option value="">-All-</option>
            ...
            $q1=$this->db->query("select * from db_users where status=1 and role_id != 1 ");
             if($q1->num_rows()>0)
             {
                 foreach($q1->result() as $res1)
               {
                 echo "<option value='".$res1->id."'>".$res1->username."</option>";
               }
             }
             else
             {
            ...
            <div class="box-body table-responsive no-padding">
              ...
              <table class="table table-bordered table-hover " id="report-data" >
                ...
                <tr class="bg-blue">
                  ...
                  <th><?= $this->lang->line('services_date'); ?></th>
                  <th><?= $this->lang->line('services_no'); ?></th>
                  <th><?= $this->lang->line('services_customer_name'); ?></th>
                  <th><?= $this->lang->line('services_technician_name'); ?></th>
                  <th><?= $this->lang->line('services_stock_used'); ?></th>
                  <th><?= $this->lang->line('services_invoice_no'); ?></th>
                  <th><?= $this->lang->line('services_expences'); ?></th>
...
function convert_excel(type, fn, dl) {
    ...
    return dl ?
        ...
        XLSX.writeFile(wb, fn || ('service-Report.' + (type || 'xlsx')));
}
  ...
  $("#view,#view_all").click(function(){
    ...
    if(from_date == "")
     ...
     if(to_date == "")
        ...
        $.post(base_url+"reports/show_service_report",{technician_id:item_id,view_all:view_all,from_date:from_date,to_date:to_date},function(result){
          //alert(result);
            setTimeout(function() {
