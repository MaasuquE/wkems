...
<div class="wrapper">
  ...
  <?php 
      ...
      $pur_total=$this->db->query("SELECT COALESCE(sum(grand_total),0) AS tot_pur_grand_total FROM db_purchase")->row()->tot_pur_grand_total;
      
      $tot_paid_amt=$this->db->select("COALESCE(SUM(paid_amount),0) AS paid_amount")->from("db_purchase")->get()->row()->paid_amount;
      ...
      $purchase_due_total=$this->db->query($query9)->row()->purchase_due;
      ...
      <div class="row">
        ...
        <div class="col-lg-3 col-xs-6">
          ...
          <div class="small-box bg-yellow">
            <div class="inner">
              <h3><?= $CI->currency($tot_paid_amt,$with_comma=true);?></h3>
             <p><?= $this->lang->line('total_paid_amount'); ?></p>
            </div>
            <div class="icon">
 ...
 /* dom:'<"row"<"col-sm-12"<"pull-left"B><"pull-right">>> <"row margin-bottom-12"<"col-sm-12"<"pull-left"l><"pull-right"fr>>>tip',*/
      buttons: {
        ...
        "footerCallback": function ( row, data, start, end, display ) {
            ...
            $( api.column( 6 ).footer() ).html(app_number_format(totol.toFixed(3)));
            $( api.column( 7 ).footer() ).html(app_number_format(paid.toFixed(3)));
            $( api.column( 8 ).footer() ).html(app_number_format(due.toFixed(3)));
