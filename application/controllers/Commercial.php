...
class Commercial extends MY_Controller
{
	...
	public function index()
	{
		...
		$data = $this->data;
		$data['page_title'] = $this->lang->line('commercial');
	...
	public function add()
	{
		$this->permission_check('sales_add');
		$data = $this->data;
		$data['page_title'] = $this->lang->line('proforma');
	...
	public function sales_save_and_update_for_modal()
	{
		$this->form_validation->set_rules('sales_date', 'Sales Date', 'trim|required');
		$this->form_validation->set_rules('customer_id', 'Customer Name', 'trim|required');

		if ($this->form_validation->run() == TRUE) {
			$result = $this->sales->verify_save_and_update_for_modal();
			echo $result;
		} else {
			echo "Please Fill Compulsory(* marked) Fields.";
		}
		...
		foreach ($list as $sales) {
			...
			if ($this->permissions('sales_add') || $this->permissions('sales_edit'))
				$str2 .= '<li>
											...
											<li>
												<a style="cursor:pointer" title="Delivery note " onclick="get_delivery_note(\'' . base_url() . 'sales/getDeliveryNote/' . $sales->id . '\',\'' . base_url() . 'sales/setDeliveryNote/' . $sales->id . '\',\'' . base_url() . 'sales/deliveryNote/' . $sales->id . '\')" >
													<i class="fa fa-fw fa-file-text text-blue"></i>Delivery note
												</a>
											</li>';
	...
	public function sendInvoiceMail($id)
	{
		...
		$hide_tems = $this->input->get('hide_items', TRUE);
		$data['hide_tems'] = explode(',', $hide_tems);
		...
		// prepare email
		$this->email
			->from('info@softlinetech.net', 'softlinetech')
			->to($invoice_email)
			->subject('Invoice')
			->message($this->load->view('mail/invoice_template', $data, true))
			->set_mailtype('html');
		if (!$this->email->send()) {
			echo "failed";
		} else {
	...
	public function print_invoice($sales_id)
	{
		...
		} else if (get_invoice_format_id() == 4) {
	...
	public function pdf($sales_id)
	{
		...
		if (get_invoice_format_id() == 3) {
			...
			$this->load->view('print-sales-invoice-4-pdf', $data);
		} else {
			$this->load->view('print-sales-invoice', $data);
		}
	...
	{
		$remarks = $this->sales->get_remarks($id);
		echo  $remarks;
		die();
	}

	public function setDeliveryNote($id)
	{
		$remarks = $this->input->post('remarks');
		$this->sales->set_remarks($id, $remarks);
	}

	public function deliveryNote($sales_id)
	{
		if (!$this->permissions('sales_add') && !$this->permissions('sales_edit')) {
			$this->show_access_denied_page();
		}
