...
class Services_model extends CI_Model {
	...
	//Datatable start
	var $table = 'db_services as a';
	var $column_order = array('a.services_no','a.id','a.services_date','a.services_customer_name','b.username','a.services_status','a.services_stock_used','a.services_expences','a.services_invoice_no','c.customer_name'); //set column field database for datatable orderable
	var $column_search = array('a.services_no','a.id','a.services_date','a.services_customer_name','b.username','a.services_status','a.services_stock_used','a.services_expences','a.services_invoice_no','c.customer_name'); //set column field database for datatable searchable 
	...
	private function _get_datatables_query()
	{
		...
		$this->db->from('db_users as b');
		$this->db->from('db_customers as c');
		//$this->db->from('db_warehouse as c');
		$this->db->where('b.id=a.services_technician_id');
		$this->db->where('c.id=a.services_customer_name');

		if($this->session->userdata('inv_userid') !=1 ){
            $this->db->where('services_technician_id = '.$this->session->userdata('inv_userid') );
        }
	...
	public function verify_and_save(){
		...
		$services_date = date('Y-m-d',strtotime($services_date));
		...
		$query1="insert into db_services(services_date,services_no,services_customer_name,services_details,services_technician_id,services_status,
		services_stock_used,services_invoice_no,services_invoice_id,services_remarks,services_returns,services_expences,
											system_ip,system_name,
											created_date,created_time,created_by)
											values('$services_date','$services_no','$services_customer_name','$services_details','$services_technician_id','$services_status',
											'$services_stock_used','$services_invoice_no','$services_invoice_id','$services_remarks','$services_returns','$services_expences',
											'$SYSTEM_IP','$SYSTEM_NAME',
	...
	public function get_details($id,$data){
		...
		$query=$this->db->query("select * from db_services where upper(id)=upper('$id')");
		if($query->num_rows()==0){
			show_404();exit;
		}
		else{
			$query=$query->row();
			$data['q_id']=$query->id;
			$data['services_date']=$query->services_date;
			$data['services_no']=$query->services_no;
			$data['services_customer_name']=$query->services_customer_name;
			$data['services_details']=$query->services_details;
			$data['services_technician_id']=$query->services_technician_id;
			$data['services_status']=$query->services_status;
			$data['services_stock_used']=$query->services_stock_used;
			$data['services_invoice_no']=$query->services_invoice_no;
			$data['services_invoice_id']=$query->services_invoice_id;
			$data['services_remarks']=$query->services_remarks;
			$data['services_returns']=$query->services_returns;
			$data['services_expences']=$query->services_expences;
			$data['opening_balance']=$query->opening_balance;


			$query_cus=$this->db->query("select customer_name from db_customers where upper(id)=upper('".$query->services_customer_name."')");
	...
	public function update_services(){
		...
		$services_date = date('Y-m-d',strtotime($services_date));
		$query1="update db_services set 
		services_date='$services_date',
		services_no='$services_no',
		services_customer_name='$services_customer_name',
		services_details='$services_details',
		services_technician_id='$services_technician_id',
		services_status='$services_status',
		services_stock_used= '$services_stock_used',
		services_invoice_no= '$services_invoice_no',
		services_remarks='$services_remarks',
		services_returns='$services_returns',
		services_expences='$services_expences'
		where id=$q_id";
		if ($this->db->simple_query($query1)){
	...
	public function update_services_not_admin(){
		//Filtering XSS and html escape from user inputs 
		extract($this->security->xss_clean(html_escape(array_merge($this->data,$_POST))));
	...
	public function update_status($id,$status){
        ...
        $query1="update db_services set status='$status' where id=$id";
        if ($this->db->simple_query($query1)){
            echo "success";
        }
        else{
	...
	public function delete_services_from_table($ids){
	
		$query1="delete from db_services where id in($ids) ";
        if ($this->db->simple_query($query1)){
            echo "success";
        }
        else{
	...
	public function show_pay_now_modal($customer_id){
		...
		$q2=$this->db->query("select * from db_services where id=$customer_id");
		$res2=$q2->row();
	    ...
	    $due_amount = number_format($customer_sales_due + $customer_opening_balance_due,3,'.','') ;
		?>
	...
	public function show_pay_return_due_modal($customer_id){
	    ...
	    $total_paid_amount = $q6->row()->total_paid_amount;
	    //$total_sales_due_amount =$total_sales_amount - $total_paid_amount;
	    $due_amount = number_format($total_sales_amount - $total_paid_amount,3,'.','') ;
		?>
