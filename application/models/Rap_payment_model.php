...
class Rap_payment_model extends CI_Model {

	//Datatable start
	var $table = 'db_rap_payment as a';
	var $column_order = array('a.rap_payment_no','a.id','a.rap_payment_date','a.rap_payment_customer_name','a.rap_payment_sum_of','a.rap_payment_on_bank','a.rap_payment_cash_or_check_no','a.rap_payment_beign_of','c.customer_name'); //set column field database for datatable orderable
	var $column_search = array('a.rap_payment_no','a.id','a.rap_payment_date','a.rap_payment_customer_name','a.rap_payment_sum_of','a.rap_payment_on_bank','a.rap_payment_cash_or_check_no','a.rap_payment_beign_of','c.customer_name'); //set column field database for datatable searchable 
	...
	private function _get_datatables_query()
	{
		...
		$this->db->where('c.id=a.rap_payment_customer_name');
	...
	public function verify_and_save(){
		...
		$rap_payment_date = date('Y-m-d',strtotime($rap_payment_date));
		...
		if( $rap_payment_no == ''){
			...
			$qs4="select coalesce(max(id),0)+1 as maxid from db_rap_payment";
			$q1=$this->db->query($qs4);
			$maxid=$q1->row()->maxid;
			$rap_payment_no=$service_init.str_pad($maxid, 4, '0', STR_PAD_LEFT);
		}
		
		//end

		$query1="insert into db_rap_payment(rap_payment_date,rap_payment_kd,rap_payment_fils,rap_payment_no,rap_payment_customer_name,rap_payment_sum_of,
		rap_payment_on_bank,rap_payment_cash_or_check_no,rap_payment_beign_of,
											system_ip,system_name,
											created_date,created_time,created_by)
											values('$rap_payment_date','$rap_payment_kd','$rap_payment_fils','$rap_payment_no','$rap_payment_customer_name','$rap_payment_sum_of',
											'$rap_payment_on_bank','$rap_payment_cash_or_check_no','$rap_payment_beign_of',
											'$SYSTEM_IP','$SYSTEM_NAME',
	...
	public function get_details($id,$data){
		...
		$query=$this->db->query("select * from db_rap_payment where upper(id)=upper('$id')");
		if($query->num_rows()==0){
			show_404();exit;
		}
		else{
			$query=$query->row();
			$data['q_id']=$query->id;
			$data['rap_payment_date']=$query->rap_payment_date;
			$data['rap_payment_kd']=$query->rap_payment_kd;
			$data['rap_payment_fils']=$query->rap_payment_fils;
			$data['rap_payment_no']=$query->rap_payment_no;
			$data['rap_payment_customer_name']=$query->rap_payment_customer_name;
			$data['rap_payment_sum_of']=$query->rap_payment_sum_of;
			$data['rap_payment_on_bank']=$query->rap_payment_on_bank;
			$data['rap_payment_cash_or_check_no']=$query->rap_payment_cash_or_check_no;
			$data['rap_payment_beign_of']=$query->rap_payment_beign_of;


			$query_cus=$this->db->query("select customer_name from db_customers where upper(id)=upper('".$query->rap_payment_customer_name."')");
	...
	public function update_rap_payment(){
		...
		$rap_payment_date = date('Y-m-d',strtotime($rap_payment_date));
		$query1="update db_rap_payment set 
		rap_payment_date='$rap_payment_date',
		rap_payment_kd='$rap_payment_kd',
		rap_payment_fils='$rap_payment_fils',
		rap_payment_no='$rap_payment_no',
		rap_payment_customer_name='$rap_payment_customer_name',
		rap_payment_sum_of='$rap_payment_sum_of',
		rap_payment_on_bank= '$rap_payment_on_bank',
		rap_payment_cash_or_check_no= '$rap_payment_cash_or_check_no',
		rap_payment_beign_of='$rap_payment_beign_of'
		where id=$q_id";
		if ($this->db->simple_query($query1)){
	...
	public function update_rap_payment_not_admin(){
		//Filtering XSS and html escape from user inputs 
		extract($this->security->xss_clean(html_escape(array_merge($this->data,$_POST))));


		$query1="update db_rap_payment set 
		rap_payment_status='$rap_payment_status',
		rap_payment_stock_used= '$rap_payment_stock_used',
		rap_payment_invoice_no= '$rap_payment_invoice_no',
		rap_payment_remarks='$rap_payment_remarks'
		where id=$q_id";
		if ($this->db->simple_query($query1)){
				$this->session->set_flashdata('success', 'Success!! Job Updated Successfully!');
				return "success";
	...
	public function update_status($id,$status){
        ...
        $query1="update db_rap_payment set status='$status' where id=$id";
        if ($this->db->simple_query($query1)){
            echo "success";
        }
        else{
	...
	public function delete_rap_payment_from_table($ids){
	
		$query1="delete from db_rap_payment where id in($ids) ";
        if ($this->db->simple_query($query1)){
            echo "success";
        }
        else{
	...
	public function show_pay_now_modal($customer_id){
		...
		$q2=$this->db->query("select * from db_rap_payment where id=$customer_id");
		$res2=$q2->row();
