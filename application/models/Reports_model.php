...
//		$this->db->group_by("c.`item_id`");
	...
	public function show_service_report(){
		extract($_POST);

		$from_date=date("Y-m-d",strtotime($from_date));
		$to_date=date("Y-m-d",strtotime($to_date));
		
		$this->db->select(array('a.services_no','a.id','a.services_date','a.services_customer_name','b.username','a.services_status','a.services_stock_used','a.services_expences','a.services_invoice_no','c.customer_name'));
	    
		if($technician_id!=''){
			
			$this->db->where("a.services_technician_id=$technician_id");
		}
		if($view_all=="no"){
			$this->db->where("(a.services_date>='$from_date' and a.services_date<='$to_date')");
		}
		$this->db->from('db_services as a');
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				if($res1->services_status== 'Processing'){ 
					$str= "<span  id='span_".$res1->id."'  class='label label-warning' style='cursor:pointer'>Processing </span>";
				}else if($res1->services_status== 'Hold'){ 
					$str= "<span  id='span_".$res1->id."'  class='label label-danger' style='cursor:pointer'>Hold </span>";
				}else{ 
					$str = "<span  id='span_".$res1->id."'  class='label label-success' style='cursor:pointer'> Completed </span>";
				}


				echo "<tr>";
				echo "<td>".++$i."</td>";
				echo "<td>".show_date($res1->services_date)."</td>";
				echo "<td>".$res1->services_no."</td>";
				echo "<td>".$res1->customer_name."</td>";
				echo "<td>".$res1->username."</td>";
				echo "<td class='text-right'>".app_number_format($res1->services_stock_used)."</td>";
				echo "<td class='text-right'>".$res1->services_invoice_no."</td>";
				echo "<td class='text-right'>".app_number_format($res1->services_expences)."</td>";
				echo "<td class='text-right'>".$str."</td>";
				echo "</tr>";
			}
