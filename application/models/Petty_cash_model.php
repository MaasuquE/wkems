...
class Petty_cash_model extends CI_Model {
	...
	//Datatable start
	var $table = 'db_petty_cash as a';
	var $column_order = array('a.petty_cash_date','a.id','a.petty_cash_ref_no','a.petty_cash_description','a.petty_cash_cash_in','a.petty_cash_cash_out','a.petty_cash_total','a.petty_cash_remarks'); //set column field database for datatable orderable
	var $column_search = array('a.petty_cash_date','a.id','a.petty_cash_ref_no','a.petty_cash_description','a.petty_cash_cash_in','a.petty_cash_cash_out','a.petty_cash_total','a.petty_cash_remarks'); //set column field database for datatable searchable 
	...
	function get_datatablesfor_parent($parent_id)
	{
		$this->_get_datatables_query();
	...
	public function verify_and_save(){
		...
		$petty_cash_date = date('Y-m-d',strtotime($petty_cash_date));
		...
		if( $petty_cash_no == ''){
			...
			$qs4="select coalesce(max(id),0)+1 as maxid from db_petty_cash";
			$q1=$this->db->query($qs4);
			$maxid=$q1->row()->maxid;
			$petty_cash_no=$service_init.str_pad($maxid, 4, '0', STR_PAD_LEFT);
		}
		
		//end

		if(!isset($parent_epetty_cash)){
			if($petty_cash_cash_in != '' && $petty_cash_cash_out == '' ){
				
				$petty_cash_total = $petty_cash_cash_in;

			}else if($petty_cash_cash_in == '' && $petty_cash_cash_out != ''){

				$petty_cash_total = $petty_cash_cash_out;

			}else if($petty_cash_cash_in != '' && $petty_cash_cash_out != ''){

				$petty_cash_total = $petty_cash_cash_in - $petty_cash_cash_out;
			...
			$query=$this->db->query("select * from db_petty_cash where parent_id=$parent_epetty_cash or id =$parent_epetty_cash ORDER BY id DESC LIMIT 1");
			
			if($query->num_rows()==0){
				$last_total = 0 ;
			}else{
				$query=$query->row() ;
				$last_total = $query->petty_cash_total;
			...
			if($petty_cash_cash_in != '' && $petty_cash_cash_out == '' ){
				...
				$petty_cash_total = $last_total+$petty_cash_cash_in;

			}else if($petty_cash_cash_in == '' && $petty_cash_cash_out != ''){

				$petty_cash_total = $last_total-$petty_cash_cash_out;

			}else if($petty_cash_cash_in != '' && $petty_cash_cash_out != ''){

				$petty_cash_total = $last_total-( $petty_cash_cash_in - $petty_cash_cash_out);

			}
			
		}
		
		$query1="insert into db_petty_cash(petty_cash_date,petty_cash_ref_no,petty_cash_description,petty_cash_cash_in,petty_cash_cash_out,petty_cash_total,
		petty_cash_remarks,system_ip,system_name,created_date,created_time,created_by,parent_id	)
											values('$petty_cash_date','$petty_cash_ref_no','$petty_cash_description','$petty_cash_cash_in','$petty_cash_cash_out','$petty_cash_total',
											'$petty_cash_remarks',
											'$SYSTEM_IP','$SYSTEM_NAME',
											'$CUR_DATE','$CUR_TIME','$CUR_USERNAME','$parent_epetty_cash')";

		
		if ($this->db->simple_query($query1)){
				$this->session->set_flashdata('success', 'Success!! New job Added Successfully!');
		        return "success";
		}
		else{
		        return "failed";
	...
	public function get_details($id,$data){
		...
		$query=$this->db->query("select * from db_petty_cash where upper(id)=upper('$id')");
		if($query->num_rows()==0){
			show_404();exit;
		}
		else{
			$query=$query->row();
			$data['q_id']=$query->id;
			$data['petty_cash_date']=$query->petty_cash_date;
			$data['petty_cash_ref_no']=$query->petty_cash_ref_no;
			$data['petty_cash_description']=$query->petty_cash_description;
			$data['petty_cash_cash_in']=$query->petty_cash_cash_in;
			$data['petty_cash_cash_out']=$query->petty_cash_cash_out;
			$data['petty_cash_total']=$query->petty_cash_total;
			$data['petty_cash_remarks']=$query->petty_cash_remarks;


			$query_cus=$this->db->query("select customer_name from db_customers where upper(id)=upper('".$query->petty_cash_customer_name."')");
			$query_cus=$query_cus->row();
			$data['customer_name']=$query_cus->customer_name;
	...
	public function update_petty_cash(){
		...
		$petty_cash_total  = 0;
		$petty_cash_date = date('Y-m-d',strtotime($petty_cash_date));
		$query1="update db_petty_cash set 
		petty_cash_date='$petty_cash_date',
		petty_cash_ref_no='$petty_cash_ref_no',
		petty_cash_description='$petty_cash_description',
		petty_cash_cash_in='$petty_cash_cash_in',
		petty_cash_cash_out='$petty_cash_cash_out',
		petty_cash_total='$petty_cash_total',
		petty_cash_remarks= '$petty_cash_remarks'
		where id=$q_id";
		if ($this->db->simple_query($query1)){
				$this->session->set_flashdata('success', 'Success!! Job Updated Successfully!');
				return "success";
		}
		else{
				return "failed";
	...
	public function update_petty_cash_not_admin(){
		//Filtering XSS and html escape from user inputs 
		extract($this->security->xss_clean(html_escape(array_merge($this->data,$_POST))));


		$query1="update db_petty_cash set 
		petty_cash_status='$petty_cash_status',
		petty_cash_stock_used= '$petty_cash_stock_used',
	...
	public function update_status($id,$status){
        ...
        $query1="update db_petty_cash set status='$status' where id=$id";
        if ($this->db->simple_query($query1)){
            echo "success";
        }
        else{
	...
	public function delete_petty_cash_from_table($ids){
	
		$query1="delete from db_petty_cash where id in($ids) or parent_id  in($ids)";
	...
	public function show_pay_now_modal($customer_id){
		...
		$q2=$this->db->query("select * from db_petty_cash where id=$customer_id");
		$res2=$q2->row();
