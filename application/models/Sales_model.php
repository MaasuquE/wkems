...
class Sales_model extends CI_Model {
	...
	//Save Sales
	public function verify_save_and_update_for_modal(){
		//Filtering XSS and html escape from user inputs 
		extract($this->xss_html_filter(array_merge($this->data,$_POST,$_GET)));
		//echo "<pre>";print_r($this->xss_html_filter(array_merge($this->data,$_POST,$_GET)));exit();
		...
		$this->db->trans_commit();
		$this->session->set_flashdata('success', 'Success!! Record Saved Successfully! '.$sms_info);
		return "success<<<###>>>$sales_id<<<###>>>$sales_code";
	...
	public function return_row_with_data($rowcount,$info){
		...
		?>
            <tr id="row_<?=$rowcount;?>" data-row='<?=$rowcount;?>'>
               ...
               <!-- Tax Amount -->
               <td style="display:none;" id="td_<?=$rowcount;?>_11">
               ...
               <!-- Tax Details -->
               <td style="display:none;" id="td_<?=$rowcount;?>_12">
	...
	public function set_remarks( $id, $remarks){
		$q1 = $this->db->where('id',$id)->update('db_sales', ['remarks'=>$remarks]);
	}
}
