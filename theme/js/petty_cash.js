$('#petty_cash_customer_name').change(function (e) {
	var v = $('#petty_cash_customer_name').val();
	$("#customer_id").select2("val", v);
...
$('#save,#update').click(function (e) {
	...
	//check_field("petty_cash_customer_name");
	//check_field("mobile");
	//check_field("state");
	...
	if (this_id == "save")  //Save start
	{
		...
		if (confirm("Do You Wants to Save Record ?")) {
			e.preventDefault();
			data = new FormData($('#petty_cash-form')[0]);//form name
			/*Check XSS Code*/
	...
	else if (this_id == "update")  //Update start
	{
		...
		if (confirm("Do You Wants to Save Record ?")) {
			...
			$.ajax({
				type: 'POST',
				url: base_url + 'petty_cash/update_petty_cash',
				data: data,
				cache: false,
				contentType: false,
				processData: false,
...
$('#savetransaction').click(function (e) {
	...
	if (confirm("Do You Wants to Save Record ?")) {
		...
		$.ajax({
			type: 'POST',
			url: $('#petty_cash-form').attr('action'),
			data: data,
			cache: false,
			contentType: false,
			processData: false,
...
//Delete Record start
function delete_petty_cash(q_id) {
	var base_url = $("#base_url").val().trim();
	if (confirm("Do You Wants to Delete Record ?")) {
		$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
		$.post(base_url+"petty_cash/delete_petty_cash", { q_id: q_id }, function (result) {
			// alert(result);return;
			if (result == "success") {
...
function multi_delete() {
	...
	if (confirm("Are you sure ?")) {
		...
		$.ajax({
			type: 'POST',
			url: base_url+'petty_cash/multi_delete',
			data: data,
			cache: false,
			contentType: false,
			processData: false,
...
function pay_now(customer_id) {
	...
	$.post($("#base_url").val().trim() + 'customers/show_pay_now_modal', { customer_id: customer_id }, function (result) {
...
function save_payment(customer_id) {
	...
	$.post('customers/save_payment', { customer_id: customer_id, payment_type: payment_type, amount: amount, payment_date: payment_date, payment_note: payment_note }, function (result) {
		result = result.trim();
		//alert(result);return;
		if (result == "success") {
			$('#pay_now').modal('toggle');
...
function pay_return_due(customer_id) {
	...
	$.post($("#base_url").val().trim() + 'customers/show_pay_return_due_modal', { customer_id: customer_id }, function (result) {
