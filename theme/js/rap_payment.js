$('#rap_payment_customer_name').change(function (e) { 
	var v = $('#rap_payment_customer_name').val();
...
$('#save,#update').click(function (e) {
	...
	//check_field("rap_payment_customer_name");
	//check_field("mobile");
	//check_field("state");
    ...
    if(this_id=="save")  //Save start
    {
					...
					if(confirm("Do You Wants to Save Record ?")){
						e.preventDefault();
						data = new FormData($('#rap_payment-form')[0]);//form name
						/*Check XSS Code*/
             ...
             // alert(result);return;
							if(result=="success")
							{
								...
								window.location=base_url+"rap_payment";
								return;
							}
	...
	else if(this_id=="update")  //Update start
    {
					...
					if(confirm("Do You Wants to Save Record ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'rap_payment/update_rap_payment',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
...
//Delete Record start
function delete_rap_payment(q_id)
{
	
   if(confirm("Do You Wants to Delete Record ?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post("rap_payment/delete_rap_payment",{q_id:q_id},function(result){
  // alert(result);return;
	   if(result=="success")
				{
