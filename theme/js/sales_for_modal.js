...
$('#modal_save,#modal_update').click(function (e) {
    ...
    var command = '';
    if(this.id == 'modal_update'){
      command = 'update';
    }

    if(this.id == 'modal_save'){
      command = 'save';
    }
    var this_id=this.id;
        ...
        $("#"+this_id).attr('disabled',true);  //Enable Save or Update button
				$.ajax({
				type: 'POST',
				url: base_url+'sales/sales_save_and_update_for_modal?command='+command+'&rowcount='+rowcount+'&tot_subtotal_amt='+tot_subtotal_amt+'&tot_discount_to_all_amt='+tot_discount_to_all_amt+'&tot_round_off_amt='+tot_round_off_amt+'&tot_total_amt='+tot_total_amt+"&other_charges_amt="+other_charges_amt,
				data: data,
				cache: false,
				contentType: false,
				processData: false,
...
function delete_sales_payment(payment_id){
 if(confirm("Do You Wants to Delete Record ?")){
  ...
  function modalSaleClose( result ){
    $('#invoice-modal').modal('hide');
    if(typeof result[2] !== 'undefined' && result[2] != '' ){
      $('#services_invoice_no').val( result[2] );
    }

    $('#services_invoice_id').val( result[1] );
